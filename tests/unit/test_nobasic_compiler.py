...
End Struct'''
        ...
        # Should contain struct definition comments
        struct_lines = [line for line in lines if 'Struct Player' in line]
        self.assertTrue(len(struct_lines) > 0)
        field_lines = [line for line in lines if 'X:' in line or 'Y:' in line or 'Health:' in line or 'Score:' in line]
        self.assertTrue(len(field_lines) >= 4)

    def test_compile_dim_struct_instance(self):
        """Test DIM var AS struct_name compilation"""
        nobasic_code = '''Struct Point
  X
  Y
End Struct
