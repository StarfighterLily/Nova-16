...
class CodeGenerator:
    ...
    def _init_builtins(self) -> Dict[str, str]:
        ...
        return {
            'set_vmode': 'builtin_set_vmode', 'set_layer': 'builtin_set_layer',
            'set_pos': 'builtin_set_pos', 'write_screen': 'builtin_write_screen',
            'read_screen': 'builtin_read_screen', 'scroll_x': 'builtin_scroll_x',
            'scroll_y': 'builtin_scroll_y', 'set_pointers': 'builtin_set_pointers',
            'write_text': 'builtin_write_text', 'set_font': 'builtin_set_font',
            'sound_play': 'builtin_sound_play', 'sound_stop': 'builtin_sound_stop',
            'set_timer': 'builtin_set_timer', 'sti': 'builtin_sti', 'cli': 'builtin_cli',
            'iret': 'builtin_iret', 'key_available': 'builtin_key_available',
            'key_read': 'builtin_key_read', 'key_clear': 'builtin_key_clear',
            'random': 'builtin_random', 'random_range': 'builtin_random_range',
            'halt': 'builtin_halt', 'enable_interrupts': 'builtin_sti', 'disable_interrupts': 'builtin_cli'
        }
    ...
    def generate(self, ast: Program) -> List[str]:
        ...
        self.assembly.append("    MOV P8, 0xFF00 ; Set stack pointer to high memory")
        self.assembly.append("    MOV P9, 0xFF00 ; Also init frame pointer")
    ...
    def find_local_vars(self, statements: List[ASTNode]) -> List[VarDecl]:
        ...
        for stmt in statements:
            if isinstance(stmt, VarDecl):
                ...
                if isinstance(stmt.init, list): # VarDecl list
    ...
    def generate_function(self, func_def: FunctionDef):
        ...
        self.assembly.append("    PUSH P9")
        self.assembly.append("    MOV P9, P8")
        
        # --- FIX: Find all local variables before allocating space ---
        all_local_decls = self.find_local_vars(func_def.body)
        if all_local_decls:
            self.assembly.append(f"    SUB P8, {len(all_local_decls)}")

        # Map parameters to positive offsets
        for i, param in enumerate(func_def.params):
            self.local_vars[param.name] = {'offset': i + 2}

        # Map local variables to negative offsets
        ...
        if func_def.name == 'timer_interrupt':
            self.assembly.append("; Return from interrupt")
            self.assembly.append("    IRET")
        elif func_def.return_type == 'void':
            self.assembly.append("; Implicit return for void function")
            self.assembly.append("    MOV P8, P9")
            self.assembly.append("    POP P9")
    ...
    def generate_block(self, body: List[ASTNode]):
        for statement in body:
            if isinstance(statement, list): # From multi-declarations
    ...
    def generate_var_decl(self, var_decl: VarDecl):
        if var_decl.value:
            ...
            offset = self.local_vars[var_decl.name]['offset']
            self.emit(f"    MOV P2, P9")
            if offset > 0: self.emit(f"    ADD P2, {offset}")
            else: self.emit(f"    SUB P2, {-offset}")
            self.emit(f"    STOR P2, {reg}")
            self.free_register()

    def generate_assignment(self, assignment: Assignment):
        self.emit_comment(f"Assignment to {assignment.name}")

        # --- FIX: Handle compound assignment ---
        # Check if the assignment value is a BinaryOp like '+=' or '-='
        if isinstance(assignment.value, BinaryOp) and assignment.value.op in ['+=', '-=']:
            op_map = {'+=': 'ADD', '-=': 'SUB'}
            
            # Get a register and load the variable's current value (LHS)
            var_reg = self.get_register()
            offset = self.local_vars[assignment.name]['offset']
            self.emit(f"    MOV P2, P9")
            if offset > 0: self.emit(f"    ADD P2, {offset}")
            else: self.emit(f"    SUB P2, {-offset}")
            self.emit(f"    LOAD {var_reg}, [P2]")

            # Evaluate the expression on the right-hand side
            rhs_reg = self.generate_expression(assignment.value.right)

            # Perform the operation (e.g., ADD) on the two registers
            ...
            self.emit(f"    STOR P2, {var_reg}")
            
            self.free_register() # Corresponds to rhs_reg
            self.free_register() # Corresponds to var_reg
        else:
            # Standard assignment (e.g., x = y + z)
    ...
    def generate_expression(self, expr: Expression) -> str:
        if isinstance(expr, Number):
            ...
            if expr.name in self.local_vars:
                offset = self.local_vars[expr.name]['offset']
                self.emit(f"    MOV P2, P9")
                if offset > 0: self.emit(f"    ADD P2, {offset}")
                else: self.emit(f"    SUB P2, {-offset}")
                self.emit(f"    LOAD {reg}, [P2]")
                return reg
            else:
            ...
            if (expr.op == '&' and isinstance(expr.right, Number) and expr.right.value in ('0xFF', '255')):
                if (isinstance(expr.left, BinaryOp) and expr.left.op == '>>' and 
                    ...
                    if val_reg.startswith('P'):
                        ...
                        # R registers are already 8-bit, just use them directly
                        self.emit(f"    MOV {result_reg}, {val_reg}")
                    self.free_register()
                    return result_reg
                else:
            ...
            reg = self.get_register()
            offset = self.local_vars[expr.left.name]['offset']
            self.emit(f"    MOV P2, P9")
            if offset > 0: self.emit(f"    ADD P2, {offset}")
            else: self.emit(f"    SUB P2, {-offset}")
            self.emit(f"    LOAD {reg}, [P2]")
    ...
    def generate_call(self, call: FuncCall) -> str:
        ...
        if call.args:
            self.emit(f"    ADD P8, {len(call.args)}")
    ...
    def generate_builtins(self):
        ...
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    MOV VM, R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_set_layer")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    MOV GL, R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_set_pos")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    POP R2"); self.emit("    MOV VX, R2"); self.emit("    MOV VY, R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_write_screen")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    SWRITE R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_read_screen")
        self.emit("    SREAD R0"); self.emit("    RET")
        self.emit_label("builtin_scroll_x")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    SROLX R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_scroll_y")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    SROLY R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_set_pointers")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    POP R2"); self.emit("    MOV P0, R2"); self.emit("    MOV P1, R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_write_text")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    POP R2"); self.emit("    TEXT R2, R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_set_font")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_sound_play")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    POP R2"); self.emit("    POP R3"); self.emit("    SPLAY R3, R2, R1"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_sound_stop")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    SPLAY R1, 0, 0"); self.emit("    PUSH R0"); self.emit("    RET")
        self.emit_label("builtin_set_timer")
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    POP R2"); self.emit("    POP R3"); self.emit("    POP R4"); self.emit("    MOV TT, R4"); self.emit("    MOV TM, R3"); self.emit("    MOV TS, R2"); self.emit("    MOV TC, R1"); self.emit("    PUSH R0"); self.emit("    RET")
        ...
        self.emit("    POP R0"); self.emit("    POP R1"); self.emit("    POP R2"); self.emit("    RNDR R0, R2, R1"); self.emit("    PUSH R0"); self.emit("    RET")
