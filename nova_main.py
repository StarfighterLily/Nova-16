...
# Event bus and decoupled peripherals
...
def initialize_system(enable_sound=True, uart_config=None):
    ...
    Phase 3: Components communicate via EventBus instead of circular back-references.
    Memory, keyboard, mouse, and timer all publish events; the InterruptController
    subscribes and dispatches interrupts.
    """
    # Create shared event bus first
    bus = EventBus()

    # Initialize components (order matters: bus-dependent first)
    mem = ram(bus=bus)
    gfx = gpu.GFX()
    ...
    # Create interrupt controller (before CPU so CPU can reference it)
    intr_ctrl = InterruptController(bus=bus, memory=mem)
    
    # Create standalone timer (subscribes to cpu.tick via bus)
    timer_dev = Timer(bus=bus, interrupt_controller=intr_ctrl)
    
    # Create CPU with event bus and interrupt controller
    proc = cpu.CPU(mem, gfx, kbd, snd, uart_device=uart_device,
                   bus=bus, interrupt_controller=intr_ctrl, timer_device=timer_dev)
    
    # Bind CPU and flags to interrupt controller (needed for _trigger)
    intr_ctrl.cpu = proc
    intr_ctrl.memory = mem
    
    # Subscribe CPU's interrupt check to post-step events
    if hasattr(intr_ctrl, 'check'):
        bus.subscribe('cpu.post_step', intr_ctrl.check)
    
    # Subscribe GFX to SCB events (for sprite dirty tracking)
