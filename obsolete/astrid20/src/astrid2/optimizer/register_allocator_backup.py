...
class GraphColoringRegisterAllocator:
    ...
    def __init__(self):
        ...
        # Expand available register sets for better allocation
        self.r_registers = ['R2', 'R3', 'R4', 'R5', 'R6', 'R7', 'R0', 'R1', 'R8', 'R9']  # Prioritize R2-R7, then temps
    ...
    def allocate_registers(self, ir_module: IRModule) -> Dict[str, str]:
        ...
        # Step 2: Build interference graph for each function with function-aware allocation
    ...
    def _allocate_function_registers(self, function: IRFunction):
        ...
        param_count = self.function_signatures.get(function.name, 0)
        ...
        if is_main:
            ...
            r_regs = self.main_r_registers.copy()
            ...
            # User-defined functions must respect calling conventions
            r_regs = self.user_function_r_registers.copy()
            p_regs = self.user_function_p_registers.copy()
            
            # For functions with parameters, allocate parameters to specific registers first
            if has_parameters:
                self._allocate_function_parameters(function, interference_graph)
                # Parameter registers are now reserved, use remaining registers for locals
                used_param_regs = self.parameter_registers[:param_count]
                # Remove used parameter registers from local allocation pool
                r_regs = [r for r in r_regs if r not in used_param_regs] + used_param_regs
    ...
    def _color_graph(self, graph: InterferenceGraph, var_type: str, available_registers: List[str]):
        """Color the interference graph using greedy graph coloring algorithm."""
