#!/usr/bin/env python3
"""
Astrid 2.0 Compiler - Main Entry Point
...
class AstridCompiler:
    """Main Astrid 2.0 compiler class."""
    ...
    def compile(self, source_code: str, filename: str = "<stdin>", verbose: bool = False) -> str:
        """
        Compile Astrid 2.0 source code to Nova-16 assembly.

        Args:
            source_code: The Astrid 2.0 source code
        ...
        try:
            ...
            if hasattr(self.semantic_analyzer, 'reset_for_new_compilation'):
                self.semantic_analyzer.reset_for_new_compilation()
            ...
            optimized_ir = self.optimizer.optimize(ir_module)
            logger.info("OK: Optimization: Register allocation and optimizations complete")

            # Phase 6: Code Generation
            assembly = self.code_generator.generate(optimized_ir)
            logger.info("OK: Code Generation: Generated Nova-16 assembly")
...
def main():
    """Main entry point for the Astrid 2.0 compiler."""
    parser = argparse.ArgumentParser(
        description="Astrid 2.0 - Nova-16 Hardware-Optimized Compiler",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m astrid2 program.ast                    # Compile to program.asm
  ...
  python -m astrid2 --help                          # Show this help
    ...
    parser.add_argument(
        "source",
        nargs="?",
        help="Astrid 2.0 source file (.ast)"
    )
    parser.add_argument(
        "-o", "--output",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"Astrid 2.0 Compiler v{__import__('astrid2').__version__}"
    )
    ...
    if args.source:
        ...
        print("Reading Astrid 2.0 source from stdin. Press Ctrl+D (Unix) or Ctrl+Z+Enter (Windows) to end input.")
        try:
