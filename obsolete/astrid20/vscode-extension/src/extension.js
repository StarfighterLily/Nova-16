...
function activate(context) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('astrid2.compile', async () => {
            ...
            if (activeEditor && activeEditor.document.languageId === 'astrid') {
                ...
                terminal.show();
                terminal.sendText(`python run_astrid.py "${fileName}"`);

                vscode.window.showInformationMessage(`Compiling ${fileName}...`);
            } else {
                vscode.window.showErrorMessage('No Astrid file is currently active');
            }
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('astrid2.run', async () => {
            const activeEditor = vscode.window.activeTextEditor;
            if (activeEditor && activeEditor.document.languageId === 'astrid') {
                const fileName = activeEditor.document.fileName;
                const binFile = fileName.replace('.ast', '.bin');

                const terminal = vscode.window.createTerminal('Astrid 2.0 Runner');
                terminal.show();
                terminal.sendText(`python nova.py "${binFile}"`);
            } else {
                vscode.window.showErrorMessage('No Astrid file is currently active');
            }
        })
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('astrid2.assemble', async () => {
            const activeEditor = vscode.window.activeTextEditor;
            if (activeEditor && activeEditor.document.languageId === 'astrid') {
                const fileName = activeEditor.document.fileName;
                const asmFile = fileName.replace('.ast', '.asm');

                const terminal = vscode.window.createTerminal('Astrid 2.0 Assembler');
                terminal.show();
                terminal.sendText(`python nova_assembler.py "${asmFile}"`);
            } else {
                vscode.window.showErrorMessage('No Astrid file is currently active');
            }
        })
